...
public void run() {
    try {
        ...
        while ((bytesLeidos = fileInputStream.read(datosEnviados)) != -1) {
            byte[] datosEnviadosCopia = new byte[bytesLeidos];
            System.arraycopy(datosEnviados, 0, datosEnviadosCopia, 0, bytesLeidos);
            DatagramPacket paqueteEnviado = new DatagramPacket(datosEnviadosCopia, bytesLeidos, IP_CLIENTE, PUERTO_CLIENTE);
